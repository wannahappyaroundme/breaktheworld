...
폰에서 손가락으로 세상을 와장창 부수며 스트레스를 푸는 모바일 웹게임입니다. 쉬운 업적부터 오래 도전할 업적까지 경험치와 꾸미기 보상을 모을 수 있고, 프로필을 만들면 여러 기기에서 기록을 이어갈 수 있습니다.
...
3. 화면 위 `LV` 버튼으로 기록책을 열어 32개 업적, 경험치, 레벨, 꾸미기 보상과 오늘의 도전을 확인합니다.
...
- 쉬움·보통·어려움·달인으로 나뉜 공개 업적 32개와 총 4,700 경험치
- 최고 20레벨, 업적 칭호 8개, 레벨별 프로필 테두리 4개와 기록책 색 3개
- 잠긴 업적도 처음부터 조건·진행도·경험치를 보여주는 전체 화면 기록책
- 하루 한 번 도전, 업적·레벨 알림과 기록 카드 공유
- 첫 접속 게스트·프로필 선택, 같은 기기의 게스트 선택 기억, 여러 기기 기록 동기화, 로그아웃
- 회원가입과 게임을 같은 밤하늘 배경·종이 카드 디자인으로 연결한 모바일 UI
- 홈 화면에 추가할 수 있는 PWA
...
- 현재 운영 검증: Vitest 789개, DB 권한·동기화 테스트 254개, 운영 빌드, 문구 검사, 타입 검사, 운영 의존성 보안 취약점 0건
...
2026년 7월 17일 업적·경험치·UI 개편을 운영에 반영했습니다. GitHub Actions 실행 `29567381477`에서 모든 검사와 배포가 성공했고, 게임·운영자 화면·새 JS/CSS 파일의 HTTP 200 응답과 390×844 모바일 화면을 확인했습니다.
...
src/progress/   업적, 경험치, 레벨, 꾸미기, 도전, 저장
